...
class SinusoidalPosEmb(nn.Module):
    def __init__(self, dim, max_t_step=1000.):
        super().__init__()
        self.dim = dim
        self.max_t_step = max_t_step

    def forward(self, x):
        x = x * (1000. / self.max_t_step)
...
class LipschitzAttention(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 32, norm=nn.GroupNorm, shape=[32,32]):
        super().__init__()
        ...
        self.to_qkv = nn.Conv2d(dim, hidden_dim * 2, 1, bias = False)
        self.to_out = nn.Conv2d(hidden_dim, dim, 1)
    ...
    def forward(self, x):
        ...
        q, v = map(lambda t: rearrange(t, 'b (h c) x y -> b h c (x y)', h = self.heads), qkv)

        q = q * self.scale

        sim = -torch.cdist(q.permute(0,1,3,2), q.permute(0,1,3,2), p=2.0)
        # sim = einsum('b h d i, b h d j -> b h i j', q, k)
...
class LipschitzLinearAttention(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 32):
        super().__init__()
        self.scale = dim_head ** -0.5
        self.heads = heads
...
class UnetLCAtten(nn.Module):
    def __init__(
        ...
        if self.random_or_learned_sinusoidal_cond:
            ...
            sinu_pos_emb = SinusoidalPosEmb(dim, max_t_step=max_t_step)
            fourier_dim = dim
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                block_klass(dim_in, dim_in, time_emb_dim = time_dim),
                block_klass(dim_in, dim_in, time_emb_dim = time_dim),
                Residual(PreNorm(dim_in, LipschitzAttention(dim_in))),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding = 1)
            ]))

        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim = time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, LipschitzAttention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim = time_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim),
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim),
                Residual(PreNorm(dim_out, LipschitzAttention(dim_out))),
                Upsample(dim_out, dim_in) if not is_last else  nn.Conv2d(dim_out, dim_in, 3, padding = 1)
            ]))
        ...
        *,
        ...
        if cifar:
            ...
            self.ds = torchvision.datasets.CIFAR10(
                root=folder, train=True, download=True, transform=transform_train)
        else:
